...
// memeOptions: [{ caption, renderedUrl, templateId, topText, bottomText }]
// onSelect: (index) => void
// onRegenerate: () => void
...
const getMemePreview = (opt) => {
      ...
      <div className="grid grid-cols-3 gap-2">
        {memeOptions?.map((opt, i) => (
          <button
            ...
            {/* Use the already-rendered Imgflip URL — no more template keyword matching */}
  ...
  {/* TOP TEXT */}
  <div className="absolute top-1 left-1 right-1 px-1 text-white text-[11px] font-extrabold text-center leading-tight drop-shadow">
    {opt.topText || ""}
  </div>

  {/* BOTTOM TEXT */}
  <div className="absolute bottom-8 left-1 right-1 px-1 text-white text-[11px] font-extrabold text-center leading-tight drop-shadow">
    {opt.bottomText || ""}
  </div>
</div>
            <div className="absolute inset-x-0 bottom-0 bg-black/60 px-1.5 py-1">
              <p className="text-white text-[10px] leading-tight text-center">{opt.caption}</p>
            </div>
            {picked === i && (
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-blue-600 transition-colors mx-auto disabled:opacity-40">
